...
export const TelStrengthValidator = function (control: AbstractControl): ValidationErrors | null {
  ...
  if(!numbers.test(value)){
    return { TelStrength: `Numer telefonu powinien zawierać tylko cyfry` };
  }

  if (value.length < 9) {
    return { TelStrength: `Numer telefonu jest za krótki` };
  }

  if (value.length > 9) {
    return { TelStrength: `Numer telefonu jest za długi` };
  }
